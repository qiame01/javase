...
public class LambdaTest6 {
    ...
    public void t4est(){//Optional示例
        ...
        Optional<Star> hhh = Optional.ofNullable(aaa).map(List::stream).orElse(Stream.empty()).filter(ggg -> StringUtils.isBlank(ggg.getSize())).findFirst();
        Star star = Optional.ofNullable(aaa).map(List::stream).orElse(Stream.empty()).filter(ggg -> StringUtils.isBlank(ggg.getSize())).findFirst().orElse(null);
